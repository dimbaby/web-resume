...
- 导入后自动把经历条目和描述要点保存到本机个人内容库，方便在精简版本中重新取用。
- 导入后先核对基本信息、解析警告和待确认内容。
- 在网页中编辑基本信息、教育经历、项目经历、技能、奖项等内容。
- 正文、主标题和副标题支持 `**加粗**`、`*斜体*` 与 `***粗斜体***` 行内 Markdown，并保留连续空格和技术字符。
- 支持模块、条目和要点拖拽排序，条目可单独或一键全部折叠后再排序。
- 支持多套模板样式、要点符号样式和无符号要点。
- 支持标准、紧凑和单页密集三档版面密度，并可调节页边距、行距、段距、姓名与联系方式间距、正文字号和条目标题字号；缩小顶部间距时，证件照和页眉占用高度会同步收紧。
- 支持主标题 / 副标题加粗、斜体控制。
- 支持复制岗位版本、重命名和删除版本。
...
导入 Markdown 或 DOCX 后，解析出的经历条目和描述要点会自动进入本机个人内容库。打开任意简历版本并展开一个模块后，可以：

- 点击模块底部条目层的“从库中添加条目”，把一整段历史经历加入当前模块。
- 点击某个完整条目内要点层的“从库中选择要点”，只挑选需要的历史要点加入当前条目。
- 点击“从其他版本选择条目 / 要点”，先指定另一份简历版本，再直接复制该版本当前的内容。

“个人内容库”和“其他简历版本”使用独立入口：前者只显示上传原件解析出的历史内容，后者按当前有效版本实时读取。服务每次启动时会重新解析应用私有目录中的 Markdown/DOCX 原件并刷新个人库，但不会覆盖网页中已经编辑的简历版本。加入时会生成当前版本自己的副本，之后编辑不会改动内容库或来源版本。完全相同的条目或要点不会被重复加入。所有内容仍只保存在本机，不会上传到外部服务。
