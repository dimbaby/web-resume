...
- 支持标准、紧凑和单页密集三档版面密度，并可调节页边距、行距、段距、正文字号和条目标题字号。
- 支持主标题 / 副标题加粗、斜体控制。
- 支持复制岗位版本、重命名和删除版本。
...
选择弹窗可以在“个人内容库”和“其他简历版本”之间切换，并支持按版本、模块类型和关键词筛选。加入时会生成当前版本自己的副本，之后编辑不会改动内容库或来源版本。完全相同的条目或要点不会被重复加入。即使某项内容已从某个简历版本中删除，仍可从个人内容库重新选回。所有内容仍只保存在本机，不会上传到外部服务。
