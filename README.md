...
- 支持标准、紧凑和单页密集三档版面密度，并可调节页边距、行距、段距和正文字号。
- 支持主标题 / 副标题加粗、斜体控制。
- 支持复制岗位版本、重命名和删除版本。
...
内容库支持按模块类型和关键词筛选；加入时会生成当前版本自己的副本，之后编辑不会改动库中的历史内容。完全相同的条目或要点不会被重复加入。即使某项内容已从某个简历版本中删除，仍可从个人内容库重新选回。内容库与简历数据一样只保存在本机，不会上传到外部服务。
